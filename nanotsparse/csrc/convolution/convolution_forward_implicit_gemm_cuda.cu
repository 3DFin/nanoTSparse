...
__global__ void __launch_bounds__(64)
    conv_forward_cuda_setting1_mode0_f16f16f32(
        int M, int K_original, int N, int kernel_volume,
        const half *__restrict__ A, const half *__restrict__ B,
        const int *__restrict__ out_in_map, half *__restrict__ C) {
  ...
  // 8 % 16) % K_original);
  const half *A_ptr = A + ((threadIdx.y * 256 % 16) % K_tile_padded) +
                      ((threadIdx.x * 8 % 16) % K_tile_padded);
  const half *B_ptr = B + (blockIdx.x % j_factors1) * 16 +
...

  {
    ...
    const int *out_in_map_ptr_local =
...
__global__ void __launch_bounds__(64)
  ...
  const half *A_ptr = A + ((threadIdx.y * 256 % 32) % K_original) +
                      ((threadIdx.x * 8 % 32) % K_original);
  const half *B_ptr = B + (blockIdx.x % j_factors1) * 16 +
                      threadIdx.y * 256 / 16 * N + threadIdx.x * 8 / 16 * N +
                      (threadIdx.x * 8) % 16;
...
__global__ void __launch_bounds__(128)
  ...
  const half *A_ptr = A + ((threadIdx.y * 256 % 32) % K_original) +
                      ((threadIdx.x * 8 % 32) % K_original);
  const half *B_ptr = B + (blockIdx.x % j_factors1) * 64 +
...

  {
    ...
    for (int ax0_ax1_fused_0_1 = 0; ax0_ax1_fused_0_1 < 2;
      ...
      // int B_kernel_offset_local = (B_kernel_offset + i2_0_0 * 32 +
      // ax0_ax1_fused_0_1 * 1024 / 64) / K_original;
      *(uint4 *)(B_shared +
...
template <int K_ld_factor, int N_ld_factor, bool K_ld_check, bool N_ld_check>
__global__ void __launch_bounds__(64)
    conv_forward_cuda_setting1_mode0_tf32tf32f32(
        int M, int K_original, int N, int kernel_volume,
        const float *__restrict__ A, const float *__restrict__ B,
        const int *__restrict__ out_in_map, float *__restrict__ C) {
  // warning: kernel could not work with K_original < 32!
  const int K_tile = 16;  // min(16, K_original);
  int K_tile_padded = K_tile * ((K_original + K_tile - 1) / K_tile);
  int K_implicit = K_tile_padded * kernel_volume;
  ...
  // * 8 % 16) % K_original);
  const float *A_ptr = A + ((threadIdx.y * 256 % 16) % K_tile_padded) +
                       ((threadIdx.x * 8 % 16) % K_tile_padded);
  const float *B_ptr = B + (blockIdx.x % j_factors1) * 16 +
                       threadIdx.y * 256 / 16 * N + threadIdx.x * 8 / 16 * N +
                       (threadIdx.x * 8) % 16;
  int reorder_loc_offset = blockIdx.x / j_factors1 * 8 * 16 +
                           (threadIdx.y % 2) * 4 * 16 + (threadIdx.x / 4);
...
__global__ void __launch_bounds__(64)
  ...
  const int *out_in_map_ptr =
      ...
      ((threadIdx.y * 256) % 32) / K_original +
      ((threadIdx.x * 8) % 32) / K_original;
  const float *A_ptr = A + ((threadIdx.y * 256 % 32) % K_original) +
                       ((threadIdx.x * 8 % 32) % K_original);
  const float *B_ptr = B + (blockIdx.x % j_factors1) * 16 +
                       threadIdx.y * 256 / 16 * N + threadIdx.x * 8 / 16 * N +
                       (threadIdx.x * 8) % 16;
...
__global__ void __launch_bounds__(128)
  ...
  const float *A_ptr = A + ((threadIdx.y * 256 % 32) % K_original) +
                       ((threadIdx.x * 8 % 32) % K_original);
  const float *B_ptr = B + (blockIdx.x % j_factors1) * 64 +
                       threadIdx.y * 256 / 64 * N + threadIdx.x * 8 / 64 * N +
                       (threadIdx.x * 8) % 64;
  int reorder_loc_offset = blockIdx.x / j_factors1 * 8 * 16 +
                           (threadIdx.y % 2) * 4 * 16 + (threadIdx.x / 4);
...
template <int K_ld_factor, int N_ld_factor, bool K_ld_check, bool N_ld_check>
__global__ void __launch_bounds__(64)
    conv_forward_cuda_setting1_mode0_f32f32f32(
        int M, int K_original, int N, int kernel_volume,
        const float *__restrict__ A, const float *__restrict__ B,
        const int *__restrict__ out_in_map, float *__restrict__ C) {
  const int K_tile = 16;
...
#pragma unroll
  ...
  const float *B_ptr = B + (threadIdx_x / (16 / 4)) * N + (blockIdx_n * 16) +
                       ((threadIdx_x * 4) % 16);
  ...
  for (int k_0 = 0; k_0 < K_loops; ++k_0) {
    {
      ...
      const int *out_in_map_ptr_local =
          out_in_map_ptr + k_0 * 16 / K_tile_padded;
      const float *A_ptr_local =
          A + (k_0 * 16 % K_tile_padded) + channel_offset_A;
...
#pragma unroll
  ...
  const float *B_ptr = B + (threadIdx_x / (64 / 4)) * N + (blockIdx_n * 64) +
                       ((threadIdx_x * 4) % 64);
