...
import nanots.backends
from nanots.utils import make_tensor
...
def build_kmap_implicit_GEMM_hashmap_on_the_fly(
    ...
    if nanots.tensor.get_allow_negative_coordinates():
    ...
    assert (
        nanots.backends.hash_rsv_ratio >= 2
    ), f"hash_rsv_ratio should be no less than 2, now {nanots.backends.hash_rsv_ratio}."

    to_insert = False
    hashmap = kmap["hashmap"]
    if hashmap is None:
        hashmap = torch.classes.nanots.GPUHashTable(_coords.shape[0] * nanots.backends.hash_rsv_ratio)
        to_insert = True
