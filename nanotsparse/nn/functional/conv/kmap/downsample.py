...
import nanotsparse
from nanotsparse.utils import make_ntuple, make_tensor
...
def spdownsample(
    ...
    sample_stride = make_tensor(sample_stride, dtype=torch.int, device=_coords.device).unsqueeze(dim=0)

    if all(stride[k] in [1, kernel_size[k]] for k in range(3)) or downsample_mode == "minkowski":
        ...
        if _coords.device.type == "cuda":
            ...
            kernel_size_t = make_tensor(kernel_size, dtype=torch.int, device=_coords.device)
            ...
            if spatial_range is not None:
                ...
                coords_max = make_tensor(coords_max_tuple, dtype=torch.int, device=_coords.device)
            else:
                coords_max = _coords.max(0).values
                coords_max[1:] = (coords_max[1:] + 2 * padding_t - (kernel_size_t - 1)) // stride_t

            if nanotsparse.tensor.get_allow_negative_coordinates():
                coords_min = _coords.min(0).values
                coords_min[1:] = torch.div(coords_min[1:] - 2 * padding_t + (kernel_size_t - 1), stride_t)
            else:
                coords_min = make_tensor((0, 0, 0, 0), dtype=torch.int, device=_coords.device)

            out_coords = torch.ops.nanotsparse.downsample_cuda(
                _coords,
                coords_max,
                coords_min,
                kernel_size_t,
